...
class ThemeBrowser extends HTMLElement {
    // noinspection JSUnusedGlobalSymbols
    connectedCallback() {
        this.attachShadow({ mode: "open" });
        this.shadowRoot.innerHTML = `
        ...
        *, *::before, *::after {
        ...
        this.fetchHocon(`/assets/theme_browser/themes.conf`).then((themes) => {
            this.themes = themes;
            ...
            this.themes.forEach((theme, id) => {
                ...
                <div class="desc">
                    <div class="title">
                        ...
                        <div class="author">by ${theme.author}</div>
                    </div>
                    <div class="description">${theme.description
                        .trim()
                        .replaceAll("<", "&lt;")
                        .replaceAll(">", "&gt;")
                        .replaceAll("\n", "<br>")}</div>
                ...
                let linksElement = element.getElementsByClassName("links").item(0);
                for (id in theme.links) {
                    let link = theme.links[id];
                    let element = document.createElement("a");
                    element.href = link;
                    element.target = "_blank";
                    element.innerHTML = `
                    <img alt="${id}" src="/assets/theme_browser/icons/${id}.svg">
                    `;
                    linksElement.append(element);
                }
    ...
    update() {
        ...
        this.themes.forEach((theme, id) => {
            let element = this.shadowRoot.getElementById("theme-" + id);

            let match = true;
            if (search) {
                if (
                    (!theme.name || !theme.name.toLowerCase().includes(search.toLowerCase())) &&
...
customElements.define("theme-browser", ThemeBrowser);
